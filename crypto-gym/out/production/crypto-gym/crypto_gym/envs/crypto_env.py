...
class CryptoEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.frames = FrameManager(framerate=5, pair_limit=pair_limit)
    ...
    def step(self, trades):
        ...
        initial_money = self.portfolio_spot_value(holdings)
        ...
        self.iter += 1
        new_money = self.portfolio_spot_value(holdings)

        self.portfolio_value_history.append(new_money)
        ...
        if self.iter % 100 is 0:
            print(time.ctime() + " " + str(self.iter) + " Current Portfolio Value: ", new_money, " Holdings: USD ", holdings[0], " BTC ", holdings[1], " EOS ", holdings[2], " ETH ", holdings[3], ".")
        return np.hstack(np.vstack(np.dstack(self.current_frames))), new_money-initial_money, self.iter is 6000 or new_money < 10, {}
    ...
    def get_bids_for_level(self, level, pair):
        return self.current_frames[4].loc[pair+book_parts[0]+book_types[level]]

    def get_asks_for_level(self, level, pair):
        return self.current_frames[4].loc[pair+book_parts[2]+book_types[level]]
        ...
        for pair in range(len(trades)):
            ...
            if trades[pair] < 0 and holdings[pair+1] > 0:  # sell crypto side / buy usd side
                ...
                for book in range(4):
                    if delta <= 0:
                        break
                    book_amounts = self.get_bids_for_level(book, name)
                    book_prices = self.create_prices_bids(level=book, closing=closing)
                    for n in range(100):
                        if book_amounts[n] > 0:
                            d = min(delta, book_amounts[n])
                            delta -= d
                            usd += d * book_prices[n]
                            holdings[pair+1] -= d
                            if delta <= 0:
                ...
                if delta > 0:
                    print("Err! Not enough order volume to buy USD for held ", delta, " and pair ", name)
                    holdings = self.trade_from_raw(holdings, trades, delta, name, pair)
                holdings[0] += usd * 0.998
            if trades[pair] > 0 and holdings[0] > 0:  # buy crypto side / sell usd side
                ...
                for book in range(4):
                    if delta <= 0:
                        break
                    book_amounts = self.get_asks_for_level(book, name)
                    book_prices = self.create_prices_asks(level=book, closing=closing)
                    for n in range(100):
                        if book_amounts[n] < 0:
                            d = min(delta, abs(book_amounts[n]*book_prices[n]))
                            delta -= d
                            crypto += d / book_prices[n]
                            holdings[0] -= d
                            if delta <= 0:
                                break
                if delta > 0:
    ...
    def portfolio_spot_value(self, holdings):
        usd = holdings[0]
        for i in range(1, len(holdings)):
            usd += holdings[i] * self.frames.get_price(i, self.iter)
        return usd
    ...
    def current_holdings(self):
        return self.current_frames[4].loc[['Candles'], :pair_limit].values[0]
