...
def parse_args():
    ...
    boolean_flag(parser, "param-noise", default=True,
    ...
    boolean_flag(parser, "distributed", default=True,
                 help="whether use distributed version  c51, https://arxiv.org/abs/1707.06887")
    parser.add_argument("--atoms", type=int, default=51, help="the atoms for distributed version")
    parser.add_argument("--vmax", type=float, default=10.0, help="max value only works when distributed version is used")
    parser.add_argument("--vmin", type=float, default=-10.0,
                        help="min value only works when distributed version is used")
    # Checkpointing
    ...
    parser.add_argument("--save-azure-container", type=str, default=None,
                        help="It present data will saved/loaded from Azure."
                             " Should be in format ACCOUNT_NAME:ACCOUNT_KEY:CONTAINER")
...
def maybe_load_model(savedir, container):
    ...
    if container is not None:
        print("Attempting to download model from Azure")
    ...
    if found_model:
        ...
        print("Loaded models checkpoint at {} iterations".format(state["num_iters"]))
        return state


if __name__ == '__main__':
    args = parse_args()
    print(args.save_dir, ['tensorboard', 'log'])
    ...
    def get_target(qt1, rewards, dones):
        atoms = args.atoms
        vmin = args.vmin
        vmax = args.vmax
        v = np.linspace(vmin, vmax, atoms)
        step = v[1] - v[0]
        batch = rewards.shape[0]
        batch_idx = np.arange(batch)
        target_p = np.zeros([batch, args.atoms])
        for idx in range(atoms):
            est = np.fmax(np.fmin(rewards + (1-dones)*v[idx] * args.gamma, v[v.size-1]), v[0])
            pos = (est-v[0]) / step
            ub = np.ceil(pos).astype(int)
            lb = np.floor(pos).astype(int)
            target_p[batch_idx, lb] += (ub - pos) * qt1[batch_idx, idx]
            target_p[batch_idx, ub] += (pos - lb) * qt1[batch_idx, idx]
        return target_p
    ...
    with U.make_session(4) as sess:
        ...
        def model_wrapper(img_in, num_actions, scope, **kwargs):
            ...
            if not args.distributed:
                args.atoms = 1
            return actual_model(img_in, num_actions, scope, layer_norm=args.layer_norm, distributed=args.distributed,
                                atoms=args.atoms, **kwargs)

        act, train, update_target, debug = rainbow.build_train(
        ...
        while True:
            ...
            if (num_iters > max(5 * args.batch_size, args.replay_buffer_size // 20) and
                ...
                if args.distributed:
                    # get t1 best q value
                    q_t1_best_final = q_t1_best(obses_tp1)
                    # get target distribution
                    target_dist = get_target(q_t1_best_final, rewards, dones)
                    # Minimize the error in Bellman's equation and compute TD-error
                    td_errors = train(obses_t, actions, rewards, obses_tp1, dones, weights, target_dist)
                else:
            ...
            if done:
                ...
                print("episodes", len(info["rewards"]))
                print("reward (100 epi mean)", np.mean(info["rewards"][-100:]))
                print("exploration", exploration.value(num_iters))
                if args.prioritized:
                    print("max priority", replay_buffer._max_priority)
                fps_estimate = (float(steps_per_iter) / (float(iteration_time_est) + 1e-6)
                                if steps_per_iter._value is not None else "calculating...")
                print("ETA: " + pretty_eta(int(steps_left / fps_estimate)))
